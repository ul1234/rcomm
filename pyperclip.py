...
print_debug_func = None

def set_print_debug_func(func):
    global print_debug_func
    print_debug_func = func
    ...
    def clipboard(hwnd):
        ...
        if False:
            ...
            while times > 0:
                ...
                if success:
                    break
                if print_debug_func: print_debug_func('set clipboard failed, try again.')
                time.sleep(0.5)
                times = times - 1
            if not success:
                raise PyperclipWindowsException("Error calling OpenClipboard")
